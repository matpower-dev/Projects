...
#export PATH="/Users/Matthew/Library/Python/2.7/bin:$PATH"
...
function __makePS1() {
    ...
    PS1+="\[${Cyan}\]@\[${LightBlue}\]\h" #  hostname
    PS1+=":\[${Green}\]\w" # working directory

    # background jobs
    local NO_JOBS=`jobs -p | wc -w`
    if [ ${NO_JOBS} != 0 ]; then
        PS1+=" \[${Yellow}\][j${NO_JOBS}]\[${Color_Off}\]"
    fi
    ...
    for screen_path in ${SCREEN_PATHS}; do
        if [ -d ${screen_path} ]; then
            ...
            if [ ${SCREEN_JOBS} != 0 ]; then
                ...
                PS1+=" \[${BBlue}\][s${SCREEN_JOBS}${current_screen}]\[${Color_Off}\]"
            fi
            break
        fi
    done
